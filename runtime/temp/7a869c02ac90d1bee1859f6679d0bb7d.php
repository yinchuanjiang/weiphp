...
<section id="contents">
   <div class="setting_step app_setting">
         <a class="step " href="<?php echo U('weixin/Publics/step_0',array('id'=>$id)); ?>">1.增加基本信息</a>
         <a class="step step_cur" href="<?php echo U('weixin/Publics/step_1',array('id'=>$id)); ?>">2.配置公众平台</a>
         <a class="step step_cur_1" href="<?php echo U('weixin/Publics/step_2',array('id'=>$id)); ?>">3.保存接口配置</a>
  </div>  
  <div class="tab-content"> 
    <!-- 表单 -->
    <form class="form-horizontal bind_step_form" method="post" action="<?php echo U('weixin/Publics/step_2',array('id'=>$id)); ?>" id="form">
      <!-- 基础文档模型 -->
      <div class="tab-pane in tab1" id="tab1">
      	<div class="item_wrap">
        <div class="form-group row cf toggle-appid">
          <label class="col-sm-2"> AppID </label>
          <div class="col-sm-8">
            <input type="text" value="<?php echo htmlentities($info['appid']); ?>" name="appid" class="text input-large">
            <span class="form-text text-muted"> （应用ID） </span>
          </div>
        </div>
        <div class="form-group row cf toggle-secret">
          <label class="col-sm-2"> AppSecret </label>
          <div class="col-sm-8">
            <input type="text" value="<?php echo htmlentities($info['secret']); ?>" name="secret" class="text input-large">
            <span class="form-text text-muted"> （应用密钥） </span>
          </div>
        </div>
        <div class="form-group row cf toggle-encodingaeskey">
          <label class="col-sm-2"> EncodingAESKey </label>
          <div class="col-sm-8">
            <input type="text" value="<?php echo htmlentities($info['encodingaeskey']); ?>" name="encodingaeskey" class="text input-large">
            <span class="form-text text-muted"> （安全模式下必填） </span>
          </div>
        </div>
        
        </div>
        <div class="form-group row cf bind_step_form_next_item">
          <div class="offset-sm-2"><button target-form="form-horizontal" type="submit" id="submit" class="btn submit-btn ajax-post">
            <?php  if($is_audit==0 && isset($reg_audit_switch) && !$reg_audit_switch) { echo '提交审核'; } else { echo '保存'; }  ?>
            
          </button>
          <span class="ml-3" style="vertical-align: -webkit-baseline-middle;">配置信息遇到问题？<a href="<?php echo U('home/index/lead'); ?>" target="_blank">点击查看接入指引</a></span></div>
        </div>
      </div>
    </form>
  </div>
  ...
  <div class="help_content">
      ...
      <p><img src="<?php echo SITE_URL; ?>/home/images/help04.png" ></p>
  </div>  
  -->
</section>
